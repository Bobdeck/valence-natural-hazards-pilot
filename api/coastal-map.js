...
module.exports=async function coastalMap(req,res){
  ...
  try{
    ...
    const outputUrls=[output.url.replace('/arcgis/','/ArcGIS/'),output.url.replace('utility.arcgisonline.com','utility.arcgis.com').replace('/arcgis/','/ArcGIS/'),output.url];let imageResponse;for(const delay of [0,750,1500,3000]){if(delay)await new Promise(resolve=>setTimeout(resolve,delay));for(const url of outputUrls){imageResponse=await fetch(url,{headers:{'User-Agent':'Mozilla/5.0 SLS-Pilot/0.11.3','Accept':'image/png,*/*'}});if(imageResponse.ok)break}if(imageResponse&&imageResponse.ok)break}if(!imageResponse||!imageResponse.ok)throw new Error('Rendered image returned HTTP '+(imageResponse&&imageResponse.status));
